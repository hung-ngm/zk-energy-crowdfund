...
/// @dev Interface to use the ZK proof in the Verifier Contract
interface IVerifier {
...
contract ZKGreenEnergyCrowdFunding is Initializable, ReentrancyGuardUpgradeable, IERC20 {
    /// @dev Guard to gate ERC20 specific functions
    modifier erc20Only() {
        require(_erc20, "Only ERC20 specific functions");
        _;
    }

    /// @dev Guard to gate ETH specific functions
    modifier ethOnly() {
        require(!_erc20, "Only ETH specific functions");
        _;
    }
    ...
    /// @dev Guard to ensure only the recipient address called
    modifier onlyRecipientGuard() {
        require(msg.sender == _recipientAddress, "Only the recipient is allowed to call this function");
        _;
    }

    /// @dev Guard to ensure the project is already zk verified its criteria
    modifier projectVerifiedGuard() {
        require(_isProjectVerified, "Project is not zk verified to be eligible to get funding");
        _;
    }
    ...
    uint256 private _minRegulatoryComplianceScore;

    uint256 private _minIPProtectionLevel;

    /// @dev The address of the verifier contract
    address private _verifierContractAddress;

    /// @dev The verified status of the project, only allow the funds to transfer if the project is verified
    bool private _isProjectVerified;
     ...
     * @param regulatoryComplianceScoreThreshold the minimum number of regulatory compliance score
     * @param ipProtectionLevelThreshold the minium number of ip protection level threshold
     * @param verifierContract the address of the verifier contract address
     */
    function initialize(
        address recipient,
        uint256 minGoal,
        ...
        _minRegulatoryComplianceScore = regulatoryComplianceScoreThreshold;

        _minIPProtectionLevel = ipProtectionLevelThreshold;
        
        _verifierContractAddress = verifierContract;

        _isProjectVerified = false;
     ...
     */
    function transferBalanceToRecipient() projectVerifiedGuard() external {
    ...
    /**
     * @return The minimum regulatory compliance score
     */
    function minRegulatoryComplianceScore() external view returns (uint256) {
        return _minRegulatoryComplianceScore;
    }

    /**
     * @return The minimum IP Protection Level
     */
    function minIPProtectionLevel() external view returns (uint256) {
        return _minIPProtectionLevel;
    }
